...
import { articleService, tagService, scheduleService } from '../services/mockApiService';
            ...
            <div className="popular-list">
              {popularTags.length > 0 ? (
                popularTags.map((tag, index) => (
                  <div key={tag.id} className="popular-item">
                    <span className="rank">#{index + 1}</span>
                    <div className="item-info">
                      <Link to={`/tags`} className="item-title">
                        {tag.name}
                      </Link>
                      <span className="item-meta">{tag.articleCount || 0} articles</span>
                    </div>
                  </div>
                ))
              ) : (
